...
def main(args):
    ...
    files = [os.path.join(vid_dir, file) for file in files]
    proc_id = int(os.environ['SLURM_PROCID'])
    ntasks = int(os.environ['SLURM_NTASKS'])
    ntasks_per_node = int(os.environ['SLURM_NTASKS_PER_NODE'])

    device = torch.device(proc_id % ntasks_per_node)
    ...
    for video_idx_idx, video_idx in enumerate(sorted(list(task_list))):
        ...
        if args.tracking_method == 'pose':
            ...
            mot = MPT(
                ...
                task_info = (proc_id, video_idx_idx, len(task_list)),
            )
        ...
        vibe_time = time.time()
        vibe_results = {}
        for person_id in tqdm(list(tracking_results.keys()), desc='proc {}, {} / {}'.format(proc_id, video_idx_idx, len(task_list))):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vid_dir', type=str, default='/mnt/lustrenew/fanhongwei1/videos_15min', 
                        help='input video path or youtube link')

    parser.add_argument('--output_folder', type=str, default='ava_output', 
